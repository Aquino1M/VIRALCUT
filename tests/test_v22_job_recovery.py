...
def test_recovery_requeues_inflight_project_with_completed_work_intact(monkeypatch,tmp_path):
    _seed(monkeypatch,tmp_path)
    queued=jobs.recover_interrupted_projects()
    p=db.fetchone("SELECT * FROM projects WHERE id='p1'")
    assert queued == ['p1']
    assert p['status']=='queued'
    assert p['progress']==65
    assert 'retomando' in p['message'].lower()
    assert db.fetchone("SELECT id FROM clips WHERE project_id='p1'")['id'] == 'c1'
